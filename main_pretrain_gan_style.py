...
from engine_pretrain_gan_style import train_one_epoch
...
def get_args_parser():
    ...
    # parser.add_argument('--resume', default='',
                        # help='resume from checkpoint') not support resume
...
def main(args):
    ...
    # define the model
    model_g = models_mae.__dict__[args.model](norm_pix_loss=args.norm_pix_loss)
    model_d = resnet50(pretrained=False, num_classes=1)
    ...
    print("Model Generator = %s" % str(model_g_without_ddp))
    print("Model Discriminator = %s" % str(model_d_without_ddp))
    ...
    param_groups = optim_factory.add_weight_decay(model_g_without_ddp, args.weight_decay)
    optimizer_g = torch.optim.AdamW(param_groups, lr=args.lr, betas=(0.9, 0.95))
    optimizer_d = torch.optim.AdamW(model_d_without_ddp.parameters(), lr=args.lr, betas=(0.9, 0.95))
    print(f'optimizer_g is {optimizer_g}')
    ...
    # misc.load_model(args=args, model_without_ddp=model_d_without_ddp, optimizer=optimizer_d, loss_scaler=loss_scaler)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # if args.output_dir and epoch + 1 == args.epochs:
            misc.save_model(
                args=args, model=model_g, model_without_ddp=model_g_without_ddp, optimizer=optimizer_g,
                loss_scaler=loss_scaler, epoch=epoch, save_name='g')
            misc.save_model(
                args=args, model=model_d, model_without_ddp=model_d_without_ddp, optimizer=optimizer_d,
                loss_scaler=loss_scaler, epoch=epoch, save_name='d')
